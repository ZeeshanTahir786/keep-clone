...
const EditArea = (props, ref) => {
  ...
  const enterHandlerForListItems = (event) => {
    if (event.key === "Enter") {
      textAreaRef.current.focus();
    }
  };

  const openLabelEditHandler = (event) => {
    event.stopPropagation();
  ...
  const closeLabelEditHandler = () => {
    if (open) {
      setlabelPopperLocation(null);
    }
  };

  const addNewChosenLabelHandler = (label) => {
  ...
  const removeLabelFromNote = (label) => {
    setChosenLabels((prevChosenLabels) => {
  ...
  const toggleLabelClickHandler = (label, checked) => {
    if (checked) {
      setChosenLabels((prevChosenLabels) => {
        return prevChosenLabels.filter((chosenLabel) => {
          return label !== chosenLabel;
  ...
  const createListToggleHandler = (item) => {
  ...
  const changeListItem = (event, index, checked) => {
    if (checked) {
      setCheckedList((prevCheckedList) => {
  ...
  const handleKeyPressForListItem = (event) => {
  ...
  const handleKeyPressForTitle = (event) => {
    if (event.key === "Enter") {
      textAreaRef.current.focus();
    }
  };
